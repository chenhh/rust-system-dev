...
* <mark style="color:red;">Send 表示資料能安全地被 move 到另一個執行緒。(多執行緒的所有權轉移)。</mark>
* <mark style="color:red;">Sync 表示資料能在多個執行緒中被同時安全地訪問。(多執行緒的借用)。</mark>

這裡“安全”指不會發生資料的競爭 (race condition)。

#### 要在 Rust 中跨執行緒共享資料，資料必須：

* 類型為 Sync （原始/不可變類型）。
* 封裝在互斥或原子類型中（Mutex、RwLock、Atomic\*） 。
* 使用像通道這樣的訊息傳遞技術來跨執行緒傳遞資料的所有權。

#### 關於 Send 的幾個關鍵點是：

* 類型 Send 可以在執行緒之間轉移所有權。
* 像 i32 和 bool 這樣的原始類型是 Send 。
* 具有內部可變的類型（如 Rc ）通常不是 Send。
* 非 Send 類型仍然可以在單個執行緒中使用，或者在包裝在像 Arc 這樣的執行緒安全的容器中時共享 。
* 跨執行緒傳輸非 Send 類型會導致未定義的行為和記憶體不安全。

## 執行緒安全

執行緒安全是指一個共享變數同時被多個執行緒執行，這個共享變數的值是一個可預期的結果而且每次執行的結果都一致的話，這個共享變數就是執行緒安全，而不會因為開啟多執行緒之後破壞了共享變數最後的結果。
