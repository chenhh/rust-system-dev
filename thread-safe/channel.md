...
Rust標準庫中還提供了另外一種執行緒之間的(同步或非同步)通信方式:mpsc(FIFO)。存儲在[`std::sync::mpsc`](https://rustwiki.org/zh-CN/std/sync/mpsc/index.html)這個模組中。
...
程式碼中的一部分(某一個執行緒)會呼叫傳送者的方法來傳送你想要傳遞的資料，然後另一部分的程式碼(另一個執行緒)會檢查接收者收到的訊息。當傳送者或接收者有一方被釋放掉時，該通道就會被關閉。
...
通道上的傳送和接收操作都將返回 Result，指示該操作是否成功。 不成功的操作通常通過將其丟棄在相應執行緒中來指示具有 “hung up” 的通道的另一半。

釋放通道的一半後，大多數操作將不再繼續進行，因此將返回 Err。 許多應用程式將繼續對該模組返回的結果進行 unwrap 處理，如果一個執行緒意外死亡，則會導致執行緒之間傳播失敗。
...
fn main() {
    ...
    // tx已經move給child thread，無法再被使用
    let handle = thread::spawn(move || {
        for i in 0..10 {
            tx.send(i).unwrap();
            println!("thread {:?} send {i}", 
                thread::current().id());
    ...
    while let Ok(r) = rx.recv() {
        println!("main thread {:?} received {r}", 
            thread::current().id());
    }
    let _ = handle.join();
}
/*
thread ThreadId(2) send 0
main thread ThreadId(1) received 0
main thread ThreadId(1) received 1
thread ThreadId(2) send 1
thread ThreadId(2) send 2
main thread ThreadId(1) received 2
thread ThreadId(2) send 3
main thread ThreadId(1) received 3
thread ThreadId(2) send 4
main thread ThreadId(1) received 4
thread ThreadId(2) send 5
main thread ThreadId(1) received 5
thread ThreadId(2) send 6
main thread ThreadId(1) received 6
thread ThreadId(2) send 7
main thread ThreadId(1) received 7
thread ThreadId(2) send 8
main thread ThreadId(1) received 8
thread ThreadId(2) send 9
main thread ThreadId(1) received 9
*/
```
...
Sender和Receiver的泛型參數必須滿足`T：Send`約束。這個條件是顯而易見的：<mark style="color:red;">被發送的消息會從一個執行緒轉移到另外一個執行緒，這個約束是為了滿足執行緒安全。</mark>如果用戶指定的泛型參數沒有滿足條件，在編譯的時候會發生錯誤，提醒我們修復錯誤。

發送者調用`send`方法，接收者調用`recv`方法，返回類型都是`Result`類型，用於錯誤處理，因為它們都有可能調用失敗。當發送者已經被銷毀的時候，接收者調用`recv`則會返回錯誤；同樣，當接收者已經銷毀的時候，發送者調用`send`也會返回錯誤。

<mark style="color:red;">在管道的接收端，如果調用</mark><mark style="color:red;">`recv`</mark><mark style="color:red;">方法的時候還沒有資料，它會進入等候狀態阻塞當前執行緒，直到接收到資料才繼續往下執行</mark>。

管道還可以是多發送端單接收端。做法很簡單，只需將發送端`Sender`複製多份即可。複製方式是調用Sender類型的clone()方法。這個庫不支持多接收端的設計，因此Receiver類型沒有clone()方法。在上例的基礎上我們稍做改動，創建多個執行緒，每個執行緒發送一個資料到接收端。
...
fn main() {
    ...
    // 此處需要drop(tx)刪除最後一個傳送者停止 `rx` 等待訊息。
    //  如果沒有drop，程式將無法停止。
    drop(tx);
...
thread ThreadId(2) send 3
thread ThreadId(2) send 4
thread ThreadId(3) send 0
thread ThreadId(3) send 1
thread ThreadId(3) send 2
thread ThreadId(3) send 3
thread ThreadId(3) send 4
main thread ThreadId(1) received 0
main thread ThreadId(1) received 1
main thread ThreadId(1) received 2
main thread ThreadId(1) received 3
...
main thread ThreadId(1) received 3
main thread ThreadId(1) received 4
thread ThreadId(4) send 0
thread ThreadId(4) send 1
thread ThreadId(4) send 2
thread ThreadId(4) send 3
thread ThreadId(4) send 4
main thread ThreadId(1) received 0
main thread ThreadId(1) received 1
main thread ThreadId(1) received 2
main thread ThreadId(1) received 3
...
<mark style="color:red;">如果需要傳輸多種類型的數據，可以為每個類型創建一個通道，你也可以使用枚舉類型來實現。</mark>
