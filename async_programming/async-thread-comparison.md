...
## Rust 異步和Linux線程上下文切換時間的比較

[https://github.com/jimblandy/context-switch](https://github.com/jimblandy/context-switch)

異步任務之間的上下文切換大約需要 0.2μs，而內核線程之間需要 1.7μs。但是，如果上下文切換是由於 I/O 就緒，那麼這個優勢就會消失： 兩者都收斂到 1.7μs。

異步任務創建新任務需要 \~0.3μs，而新內核線程需要 \~17μs。

異步任務的每個任務（即對於執行不多的任務）的記憶體消耗大約為幾百位元組，而內核線程的記憶體消耗約為 20KiB（9.5KiB 使用者，10KiB 內核）。這是一個最低限度：要求更高的任務自然會使用更多。
