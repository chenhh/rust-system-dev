...
在每一個函數中都獲取所有權並接著返回所有權有些囉嗦。<mark style="color:blue;">如果我們想要函數使用一個值但不獲取所有權時，可使用引用(借用)</mark>。
...
在 Rust 中，所有值預設都是堆疊分配的。通過創建 `Box<T>`，可以把值裝箱（boxed）來 使它在堆積上分配。箱子（box，即 `Box<T>` 類型的實例）是一個智慧指針，指向堆積分配的 T 類型的值。當箱子離開作用域時，它的解構函數會被調用，內部的對象會被銷毀，堆積上分配的內存也會被釋放。
